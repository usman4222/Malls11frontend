...
import { CREATE_GIG_FAIL, CREATE_GIG_REQUEST, CREATE_GIG_SUCCESS, DELETE_GIG_FAIL, DELETE_GIG_REQUEST, DELETE_GIG_SUCCESS, GET_MY_GIGS_FAIL, GET_MY_GIGS_REQUEST, GET_MY_GIGS_SUCCESS, GET_USER_GIGS_REQUEST, GET_USER_GIGS_SUCCESS, GET_USER_GIGS_FAIL, CHANGE_GIG_STATUS_REQUEST, CHANGE_GIG_STATUS_SUCCESS, CHANGE_GIG_STATUS_FAIL, GET_GIG_REQUEST, GET_GIG_SUCCESS, GET_GIG_FAIL } from "../../store/slices/gig/gigSlice";
...
export const changeGigStatus = (gigId, newStatus) => async (dispatch) => {
    dispatch(CHANGE_GIG_STATUS_REQUEST());

    try {
        const { data } = await axiosInstance.patch(`/gig/change-gig-status/${gigId}`, { status: newStatus });

        dispatch(CHANGE_GIG_STATUS_SUCCESS(data.data));
    } catch (error) {
        dispatch(
