...
export default function VerifyAccountEmail() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await axios.post(
                `${backendUrl}/auth/register/verify-account?tempToken=${tempToken}`,
                {
                    email,
                    otp,
                }
    ...
    const handleResendOtp = async () => {
        setIsResending(true);
        try {
            const res = await axios.post(`${backendUrl}/auth/register/resendOtp`, { email });

            if (res.data.success) {
                const newToken = res.data.newTempToken;
                setTempToken(newToken);
                localStorage.setItem("tempToken", newToken);
                setSearchParams({ tempToken: newToken }); // update URL query param
